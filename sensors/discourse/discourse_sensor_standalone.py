#!/usr/bin/env python3
"""
Discourse Forum Sensor for KOI System - Standalone Version
Collects discussions from Regen Network Discourse forums
Works without KOI client dependency for testing
"""
...
class DiscourseSensor:
    ...
    async def collect_forum(self, forum_config: Dict, limit: int = 50) -> List[Dict]:
        ...
        if 'all' in forum_config.get('categories', ['all']):
            ...
            category_slugs = [cat['slug'] for cat in categories[:10] if 'slug' in cat]
        else:
    ...
    async def run(self, limit_per_forum: int = 20):
        ...
        """
        print("=" * 60)
        print("🌐 DISCOURSE FORUM SENSOR (Standalone)")
        print("=" * 60)
...
async def main():
    ...
    async with DiscourseSensor() as sensor:
        documents = await sensor.run(limit_per_forum=5)
        return documents
